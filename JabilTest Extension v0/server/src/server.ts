...
interface SilkTestSettings {
    maxNumberOfProblems: number;
    enableAutoComplete: boolean;
    enableErrorChecking: boolean;
    enableIntelliSense: boolean;
    languagePath: string;
    registeredDLLs: string[];
}

// Configuración global
const defaultSettings: SilkTestSettings = {
...
let globalSettings: SilkTestSettings = defaultSettings;

// Cache de configuración de documentos
const documentSettings: Map<string, Thenable<SilkTestSettings>> = new Map();

// Funciones estándar de SilkTest/4Test
const standardFunctions = new Map<string, FunctionInfo>();
...
function initializeStandardFunctions(): void {
...
connection.onDidChangeConfiguration(change => {
    if (hasConfigurationCapability) {
        ...
        globalSettings = <SilkTestSettings>(
            (change.settings.silktestLanguageServer || defaultSettings)
        );
    }
...
function getDocumentSettings(resource: string): Thenable<SilkTestSettings> {
    if (!hasConfigurationCapability) {
...
connection.onNotification('dll/registered', (params: { dllPath: string; dllName: string }) => {
...
connection.onNotification('dll/unregistered', (params: { dllName: string }) => {
